...
{
    public class HungryWarden : ModProjectile
    {

        private const string ChainTexturePath = "OvermorrowMod/Projectiles/Piercing/HungryDraw";
		...
		{		
            ...
            if (!Main.player[projectile.owner].channel || BetweenBack > 500f || ComingBackTimer > 100 && !StickingToNPC)
			{
				ReturnBack();
			}
		...
		private void UpdateStickyJavelins(NPC target)
		{
				...
				for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
				{
					...
					if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
						...
						&& currentProjectile.modProjectile is HungryWarden javelinProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
		...
		private void StickyAI()
		{
			...
			bool hitEffect = projectile.localAI[0] % 30f == 0f;
				...
				if (penet2 > 9)
				{
					penet = 0;
					penet2 = 0;
					Main.npc[projTargetIndex].StrikeNPC(projectile.damage + Main.npc[projTargetIndex].defense, 0, 0);
					penet3++;
				}
				{
