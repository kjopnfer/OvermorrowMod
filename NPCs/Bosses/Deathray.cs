...
namespace OvermorrowMod.NPCs.Bosses
{
    public abstract class Deathray : ModProjectile
    {
        ...
        {
            spriteBatch.Draw(LaserBeginTexture, projectile.Center - Main.screenPosition, null, beamColor * projectile.Opacity, projectile.rotation, new Vector2(LaserBeginTexture.Width * scale, LaserBeginTexture.Height) / 2, new Vector2(scale, 1f), SpriteEffects.None, 0f);
            float middleLaserLength = LaserLength;
            middleLaserLength -= (LaserBeginTexture.Height / 2 + LaserEndTexture.Height) * scale;
            ...
            if (middleLaserLength > 0f)
            {
                ...
                {
                    spriteBatch.Draw(LaserMiddleTexture, centerOnLaser - Main.screenPosition, null, beamColor * projectile.Opacity, projectile.rotation, new Vector2(LaserMiddleTexture.Width, LaserMiddleTexture.Height) / 2f, new Vector2(scale, 1f), SpriteEffects.None, 0f);
            ...
            if (Math.Abs(LaserLength - 2000) < 30f)
            {
                ...
                spriteBatch.Draw(LaserEndTexture, laserEndCenter, null, beamColor * projectile.Opacity, projectile.rotation, new Vector2(LaserEndTexture.Width, LaserEndTexture.Height) / 2f, new Vector2(scale, 1f), SpriteEffects.None, 0f);
            }
        }
        ...
        public override void CutTiles()
        {
            ...
            Terraria.Utils.PlotTileLine(projectile.Center, projectile.Center + unit * LaserLength, projectile.Size.Length() * projectile.scale, DelegateMethods.CutTiles);
        }
