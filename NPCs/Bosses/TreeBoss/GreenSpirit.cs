...
namespace OvermorrowMod.NPCs.Bosses.TreeBoss
{
    ...
    public class GreenSpiritP2 : ModProjectile, ITrailEntity
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[1] == 0)
            {
                ...
                {
                    Vector2 RandomVelocity = Vector2.One.RotatedByRandom(MathHelper.TwoPi) * Main.rand.Next(5, 15);
                    Dust dust = Main.dust[Terraria.Dust.NewDust(projectile.Center, 2, 2, 107, RandomVelocity.X, RandomVelocity.Y, 0, new Color(0, 255, 242), 0.5f)];
            ...
            if (projectile.timeLeft > 80 * 10)
            {
                ...
                double RotationCounter = MathHelper.Lerp(StartingRotation, StartingRotation + MathHelper.TwoPi, Utils.Clamp(projectile.ai[0]++, 0, 100f * 10) / (100f * 10));
                projectile.Center = RotationCenter.Center + new Vector2(Radius, 0).RotatedBy(RotationCounter);
                OldPosition = RotationCenter.Center;
            }
            else
            {
                if (Converge && projectile.timeLeft == 20 * 10)
                {
                    Main.PlaySound(SoundID.DD2_PhantomPhoenixShot, projectile.Center);
                    projectile.velocity = Vector2.Normalize(OldPosition - projectile.Center) * 12;
                }
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            spriteBatch.Draw(texture, projectile.Center - Main.screenPosition, new Rectangle?(rect), Color.Cyan * 0.5f, projectile.rotation + MathHelper.PiOver2, drawOrigin, new Vector2(0.4f, 2f), SpriteEffects.None, 0);
            spriteBatch.Draw(texture, projectile.Center - Main.screenPosition, new Rectangle?(rect), Color.Cyan * 0.5f, projectile.rotation, drawOrigin, new Vector2(0.4f, 1f), SpriteEffects.None, 0);
        }

        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            Texture2D SoulTexture = ModContent.GetTexture("OvermorrowMod/Textures/Extra_89");
            Main.spriteBatch.Draw(SoulTexture, projectile.Center - Main.screenPosition, null, Color.Cyan, projectile.rotation + MathHelper.PiOver2, SoulTexture.Size() / 2, new Vector2(0.5f, 1), SpriteEffects.None, 0f);
        ...
        { 
            ...
            {
                Vector2 RandomVelocity = Vector2.One.RotatedByRandom(MathHelper.TwoPi) * Main.rand.Next(5, 15);
                Dust dust = Main.dust[Terraria.Dust.NewDust(projectile.Center, 2, 2, 107, RandomVelocity.X, RandomVelocity.Y, 0, new Color(0, 255, 242), 1f)];
                dust.shader = GameShaders.Armor.GetSecondaryShader(16, Main.LocalPlayer);
            }
        }
    }

    public class GreenSpiritP2Circle : ModProjectile, ITrailEntity
    {
        public Color TrailColor(float progress) => Color.Cyan;
        public float TrailSize(float progress) => 20;
        public bool TrailActive() => true;
