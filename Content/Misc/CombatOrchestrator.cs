...
namespace OvermorrowMod.Content.Misc
{
    public class CombatOrchestrator : ModNPC
    {
        ...
        private const int OpeningBuildupDuration = 90;
        ...
        public void ClaimReward(int itemId)
        {
        ...
        public override void AI()
        {
            ...
            {
                cleanupTimer++;
                if (cleanupTimer >= CleanupDuration)
                {
                    NPC.active = false;
                }
                return;
            ...
            if (isRevealed)
            {
                ...
                {
                    float progress = (OpeningBuildupDuration - openingBuildupTimer) / (float)OpeningBuildupDuration;
        ...
        {
            bool bothClosed = left.State == CombatDoor_TE.DoorState.Closed &&
        ...
        private void UpdateEnd()
        {
            ...
            bool playerInRange = Main.LocalPlayer.active && Vector2.Distance(Main.LocalPlayer.Center, NPC.Center) <= interactRange;
        ...
        private static void SpawnRedBurst(Vector2 position)
        {
            ...
            Texture2D sparkTex = ModContent.Request<Texture2D>(AssetDirectory.Textures + "trace_01", AssetRequestMode.ImmediateLoad).Value;

            var pulse = new Circle(circleTex, ModUtils.SecondsToTicks(0.4f), canGrow: true, useSineFade: true)
            {
                fadeIn = false,
                floatUp = false,
                doWaveMotion = false,
            ...
            ParticleManager.CreateParticleDirect(pulse, position, Vector2.Zero, Color.Red, 1f, 0.6f, 0f, useAdditiveBlending: true);

            for (int i = 0; i < 14; i++)
            {
                Vector2 vel = Vector2.UnitX.RotatedByRandom(MathHelper.TwoPi) * Main.rand.NextFloat(3f, 6f);
                float scale = Main.rand.NextFloat(2f, 5f);
                var spark = new Spark(sparkTex, 0f, true, 0f) { endColor = Color.DarkRed };
                ParticleManager.CreateParticleDirect(spark, position, vel, Color.Red, 1f, scale, 0f, useAdditiveBlending: true);
            }
        }
        ...
        private void EnterEnd()
        {
            ...
            var bigPulse = new Circle(pulseTex, ModUtils.SecondsToTicks(0.9f), canGrow: true, useSineFade: true)
            {
                fadeIn = false,
                floatUp = false,
                doWaveMotion = false,
            ...
            ParticleManager.CreateParticleDirect(bigPulse, NPC.Center, Vector2.Zero, Color.Gold, 1f, 1.2f, 0f, useAdditiveBlending: true);

            var innerPulse = new Circle(circleTex, ModUtils.SecondsToTicks(0.6f), canGrow: true, useSineFade: true)
            {
                fadeIn = false,
                floatUp = false,
                doWaveMotion = false,
            ...
            ParticleManager.CreateParticleDirect(innerPulse, NPC.Center, Vector2.Zero, Color.LightGoldenrodYellow, 1f, 0.7f, 0f, useAdditiveBlending: true);

            for (int i = 0; i < 22; i++)
            {
                Vector2 vel = Vector2.UnitX.RotatedByRandom(MathHelper.TwoPi) * Main.rand.NextFloat(4f, 8f);
        ...
        private void SpawnImpactParticles()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                var spark = new Spark(sparkTex, maxTime: Main.rand.Next(15, 30), true, 0f) { endColor = Color.Goldenrod };
                ParticleManager.CreateParticleDirect(spark, NPC.Bottom, vel, Color.Gold, 1f, scale, 0f, ParticleDrawLayer.BehindProjectiles, useAdditiveBlending: true);
            }
        }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            else if (hasImpacted && !isOpen && frame == 0) // Idle wobble while closed
            {
                if (wobbleTimer == 0)
                {
                ...
                if (wobbleTimer < wobbleActiveTicks)
                {
                    ...
                    NPC.rotation = (float)System.Math.Sin(wobbleTimer * 0.5f) * wobbleStrength * damping * wobbleDirection;
                }
                else
                {
                    NPC.rotation = 0f;
            ...
            float cleanupAlpha = isCleaningUp ? 1f - MathHelper.Clamp(cleanupTimer / (float)CleanupDuration, 0f, 1f) : 1f;
            ...
            spriteBatch.Draw(tex, NPC.Center - screenPos, drawRectangle, drawColor * cleanupAlpha, NPC.rotation, drawRectangle.Size() / 2f, 1f, SpriteEffects.None, 0f);
            return false;
        }
        ...
        private void EmitRays()
        {
            ...
            Texture2D rayTex = ModContent.Request<Texture2D>(AssetDirectory.Textures + "ray", AssetRequestMode.ImmediateLoad).Value;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                ParticleManager.CreateParticleDirect(ray, NPC.Center, Vector2.Zero, Color.Gold, 1f, scale, rotation, ParticleDrawLayer.BehindNPCs, useAdditiveBlending: true);
            }
        }
    }
}
