...
namespace OvermorrowMod.Common.Primitives.Trails
{
    ...
    {
        public FireTrail() : base(42, ModContent.Request<Texture2D>(AssetDirectory.Trails + "Fire", AssetRequestMode.ImmediateLoad).Value, true)
        {
        }
        ...
        public override void PrepareTrail()
        {
            ...
            for (int i = 0; i < Positions.Count - 1; i++)
            {
                ...
                else
                {
                    ...
                    // Compute scrolling offset based on game time
                    float scrollSpeed = 0.5f; // Adjust the speed of scrolling
                    float time = (float)Main.GameUpdateCount / 10f; // Replace with your game time logic
                    float scrollOffset = (time * scrollSpeed) % 1.0f; // Loop the offset when it exceeds 1.0

                    // Adjust texture coordinates with the scrolling offset
                    float startOffsetScrolled = (startOffset + scrollOffset) % 1.0f;
                    float endOffsetScrolled = (startOffset + segmentSize + scrollOffset) % 1.0f;

                    AddVertex(pos1 + off1, col1, new Vector2(startOffsetScrolled, 0f));
                    AddVertex(pos1 - off1, col1, new Vector2(startOffsetScrolled, 1f));
                    AddVertex(pos2 + off2, col1, new Vector2(endOffsetScrolled, 0f));

                    AddVertex(pos2 + off2, col1, new Vector2(endOffsetScrolled, 0f));
                    AddVertex(pos2 - off2, col1, new Vector2(endOffsetScrolled, 1f));
                    AddVertex(pos1 - off1, col1, new Vector2(startOffsetScrolled, 1f));
                    ...
                    //AddVertex(pos1 + off1, col1, new Vector2(0, 0));
                    //AddVertex(pos1 - off1, col1, new Vector2(0, 1));
                    //AddVertex(pos2 + off2, col1, new Vector2(1, 0));

                    //AddVertex(pos2 + off2, col1, new Vector2(1, 0));
                    //AddVertex(pos2 - off2, col1, new Vector2(1, 1));
                    //AddVertex(pos1 - off1, col1, new Vector2(0, 1));
                }
            }
        }
