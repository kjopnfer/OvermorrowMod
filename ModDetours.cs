    ...
    public static class ModDetours
    {
        ...
        private static void DrawNPCChatButtons(On.Terraria.Main.orig_DrawNPCChatButtons orig, int superColor, Color chatColor, int numLines, string focusText, string focusText3)
        {
            if (Main.LocalPlayer.talkNPC != -1)
            {
                ...
                // This is the close button, I think
                if (Main.npcChatFocus1 && Main.mouseLeft && Main.mouseLeftRelease)
                {
                    //ClickedButton = false;
                }
                    ...
                    if (MouseCursor.Between(TextPosition, TextPosition + StringSize * TextScale) && !PlayerInput.IgnoreMouseInterface)
                    {
                        ...
                        if (Main.mouseLeft && Main.mouseLeftRelease)
                        {
                            ...
                            if (!AcceptButton)
                            {
                                ...
                                {
                                    "dude wtf i heard aphid is buying nfts",
                                    "i thought it was a joke",
                                    "but it wasnt",
                                    "i cant believe hes done this",
                                    "holy shit",
                                    "u gotta stop him"
                                };*/
                                //var QuestDialogue = curren
                                ...
                                /*if (DialogueCounter < QuestDialogue.Count)
                                {
                                    Main.PlaySound(SoundID.MenuTick);
                                    Main.npcChatText = QuestDialogue[DialogueCounter++];
